...
- a future offline ingest path for downloaded Twitter/X account archives
...
- Query ID (query hash) auto-discovery from Twitter/X web JS bundles with TTL cache + static fallback IDs
...
These are our architectural choices, made to serve tweetnook's goals (unattended sync, raw data preservation, embedded search, attention-export integration). They are independent decisions, not inherited from any existing tool.
...
- **Storage module shape**: use `tweetnook/storage/backend.py` as the concrete backend module. The LanceDB design changes storage semantics enough that pretending we have a thin interchangeable database adapter would be misleading; keep one real backend implementation behind the `ArchiveStore` API.
- **Primary capture approach**: Direct GraphQL API calls (httpx, async), not Playwright interception.
- **Query IDs**: Auto-discover query IDs from Twitter/X web JS bundles with an on-disk TTL cache + fallback static IDs (avoid manual weekly updates).
...
- **Built-in scheduling**: the always-on Web service owns persisted scheduled syncs with hourly,
  ...
  history contract as Web and manual CLI runs. Fixed-time cadences may apply a validated
  nonnegative random delay after the selected run time by default when the next launch is
  calculated; the setting can be disabled for an exact selected time, and the chosen timestamp
...
- **Optional automated tagging**: direct Gemini support is installed with the
  `automated-tagging` extra and managed on its own conditional Settings page. Core sync, Web, and
  manual tag CRUD/search remain independent of provider packages. One live-catalog Gemini model
  handles distinct text/media prompts and schemas; image-generation families such as Nano Banana
  are omitted, while other model capabilities are documented for the user to verify. Free processing groups homogeneous batches of up
  to 20 tweets through `generateContent`, honors configured RPM/RPD, and disables Search. Paid uses
  one Interactions request per tweet with an internal sequential wave ceiling for now, Standard/Flex,
  an estimated daily spend control backed by a cached OpenRouter pricing snapshot, an append-only JSONL
  request ledger plus atomic summary, and a conservative local monthly Search-query cutoff. Existing
  archive tags are bounded prompt context rather than a function tool.
  Its Settings page can
  search/select one archived tweet and run the current form values through the same no-tag-write
  preview behavior as CLI `--test`, including unsaved form edits.
...
## Twitter/X API Reverse Engineering

The Twitter/X web app uses an undocumented internal GraphQL API. Several areas require reverse-engineered knowledge. We consult existing open-source tools in `reference/` as **empirical data sources for Twitter/X API behavior**, not as architectural or code references.

> **Rule**: if the only rationale for a design choice is “another tool does it this way”, stop and justify it against our own requirements or delete it. Reference code is for understanding Twitter/X’s undocumented API, not for copying implementation patterns.

Key areas where prior art provides useful empirical data:

1. **Query ID rotation** — Twitter/X periodically rotates GraphQL query hashes. Known approaches in the wild:
...
We rely on explicitly supplied Twitter/X session values:
- `auth_token` (session)
...
1. **Env vars**: `TWEETNOOK_AUTH_TOKEN`, `TWEETNOOK_CT0`, `TWEETNOOK_USER_ID` (numeric)
2. **Config file**: `~/.config/tweetnook/config.toml` (`[auth]` section)

The Web UI's Settings → Setup pane writes the config values and can test them. The server never
reads local browser data, which supports deployments where tweetnook and the user's browser run
on different machines.

User ID resolution:
- Set explicitly via `TWEETNOOK_USER_ID` env var or `user_id` in config
...
- Refresh behavior:
  ...
  - Provide `tweetnook auth refresh-ids` to force refresh.
...
- Validate the resolved owner identity against local DB metadata if the archive already exists; refuse to mix two Twitter/X accounts into one archive.

Command semantics:
- `tweetnook auth check` runs this shared preflight without creating or writing the DB; it reports which collections are ready.
- `tweetnook auth check` exit codes: `0` = all probed collections ready, `1` = local auth/config input missing or inconsistent, `2` = remote/API validation failed for at least one probed collection.
- `tweetnook sync all` is **all-or-nothing at preflight time**: if bookmarks or likes fail validation, abort before syncing either collection.
- Once `tweetnook sync all` has started syncing, it is **not** cross-collection atomic: if bookmarks finish and likes later fail, bookmark writes remain committed and the command exits non-zero with a partial-failure summary.
...
- The shipped backend stores data in LanceDB in `tweetnook/storage/backend.py`.
...
- Renamed the concrete storage implementation to `tweetnook/storage/backend.py`, matching the fact that we carry one real backend rather than a generic SQL adapter.
...
- Keep raw GraphQL payloads as the source of truth. If extraction is incomplete or Twitter/X changes shape again, we can rehydrate from stored `raw_json`.
...
Current verification from the public Twitter/X web client on 2026-03-15:
...
- Older archives can be refreshed through a timeline rewalk mode (`tweetnook sync ... --article-backfill`) before we add a narrower tweet-level article fetch path.
- Key `article` rows by source tweet id and preserve the full raw tweet/article block for later migration.
- The current targeted refresh path is authenticated `TweetDetail` (`tweetnook articles refresh`); no Playwright-only article fallback is needed unless `TweetDetail` regresses back to preview-only payloads later.
...
We want a second ingestion path for downloaded Twitter/X account archives. A fresh sample is available locally in `data/`; see `docs/ANALYSIS-archive-import.md` for the concrete file inventory and merge notes.
...
- `tweets.js` rows are close enough to Twitter/X legacy tweet payloads that archive import should adapt them into the existing tweet/extractor path rather than inventing a second tweet model.
...
- `tweets_media/` should be copied into the managed tweetnook media layout during import, then registered against `media` rows via `local_path` / `download_state`.
...
- Shipped import behavior: if auth is available, `tweetnook import x-archive ...` runs bulk `tweets` / `likes` reconciliation and then processes one stable snapshot of all sparse `TweetDetail` rows eligible at the start of that phase. `--no-enrich` skips the per-item phase, `--detail-lookups N` bounds one import invocation, and bare `tweetnook import enrich` selects every currently eligible row without an internal page cap while retaining 100-attempt write flushes and rate-limit pacing.
...
- Current SQLite schema-v4 databases open through a version-only fast path with no integrity scan,
  ...
  without copying unchanged canonical archive data. `db check` owns explicit integrity scans.
- Currently available tweet objects share the `done`/`resurrected` state set for downstream consumers such as tagging and Web health. Any successful live timeline ingestion clears prior unavailable reason/detail and scheduler metadata. Successful resurrection stores every thread tweet and relation returned by the detail response, while any older thread-expansion marker that predates a separate recovery is treated as stale and queued for a fresh context pass.
- The Web presentation derives availability from canonical `tweet_object.enrichment_state` and `enrichment_reason`, using captured tombstones or recognizable legacy system text only when canonical state is incomplete. Feed rows are never removed for availability: direct tweets and missing thread, quote, or retweet targets retain their position as typed placeholders, while `done`/`resurrected` content replaces stale tombstone payloads. Missing text is marked independently so retained media/actions remain usable, and raw Twitter/X media metadata supplies a non-download placeholder when older rows lack normalized media records.
...
```
tweetnook sync bookmarks            # incremental
tweetnook sync likes                # incremental
tweetnook sync tweets               # your own authored tweets
tweetnook sync all                  # preflight both, then sync bookmarks + likes
tweetnook sync all --full           # full resync (ignore duplicates)
tweetnook sync bookmarks --limit 5  # stop after 5 pages (testing)

tweetnook view bookmarks            # show recent bookmarks in the terminal
tweetnook view likes                # show recent likes in the terminal
tweetnook view tweets               # show your authored tweets in the terminal

tweetnook export json               # (phase 2+) export all collections
tweetnook media download            # fetch archived media files
tweetnook unfurl                    # fetch final/canonical URL metadata
tweetnook articles refresh          # refresh article-bearing tweets via TweetDetail
tweetnook threads expand            # capture parent/context tweets + linked status URLs
tweetnook threads expand --refresh TWEET_ID  # re-fetch an explicit target

tweetnook auth check                # run shared preflight, report local + remote readiness
tweetnook auth refresh-ids          # force query id refresh

tweetnook import x-archive ARCHIVE  # zip or extracted directory
tweetnook import x-archive ARCHIVE --no-enrich
tweetnook import x-archive ARCHIVE --detail-lookups 100
tweetnook import enrich             # all currently eligible sparse rows
```
...
data. `tweetnook stats` renders a responsive Rich bento grid from those sections; related tiles
...
On first invocation, tweetnook:
1. Auto-creates XDG directories (`~/.config/tweetnook/`, `~/.local/share/tweetnook/`, `~/.cache/tweetnook/`)
...
`tweetnook auth check` uses the same preflight path as sync before any data is written.

Reserved for future (not implemented in MVP):
- `tweetnook sync ... --playwright` (adapter fallback)
...
- [x] Linked Twitter/X-status URL and quote-relation expansion under one `max_linked_depth` limit
...
- [x] Twitter/X archive import (`tweetnook import x-archive ...`)
...
- [x] Improve long-running runner observability where network retries make the CLI look stalled.
  - Next cleanup item landed: `tweetnook threads expand` now prints phase/progress output plus visible 429 retry/cooldown diagnostics so large thread-expansion jobs no longer look dead while the HTTP layer is backing off.
- [x] Reduce startup silence and unnecessary preload scans in `tweetnook threads expand`.
...
- [x] Decouple post-sync auto-embedding from sync success.
  - Next cleanup item landed: archive capture success now wins. If auto-embedding fails, sync warns and leaves the new tweets for a later `tweetnook embed` run or the next sync instead of failing the completed sync.
...
5. **URL snapshot runner**: inline CLI commands landed for the first pass (`tweetnook media download`, `tweetnook unfurl`); decide later whether ArchiveBox/snapshotting needs a queue table or can stay command-driven.
...
7. **Bookmark archive coverage**: the 2026-03-16 Twitter/X archive sample contains likes, authored tweets, deleted tweets, and exported media, but no bookmark dataset. Is bookmarks export unsupported by Twitter/X, or just absent from this specific archive?

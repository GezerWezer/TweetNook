...
import tweetnook.sync as sync_module
...
from tweetnook.client.timelines import TimelineTweet
...
from tweetnook.query_ids import QueryIdStore
...
async def test_run_auto_followups_continues_after_task_failure(
    ...
    import tweetnook.resurrection as resurrection_module
...
@pytest.mark.asyncio
async def test_sync_remains_successful_when_automated_tagging_followup_fails(
    paths,
    config: AppConfig,
    auth_bundle,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    async def fail_tagging(**_kwargs):
        raise RuntimeError("Gemini unavailable")

    monkeypatch.setattr(sync_module, "_run_followup_tagging", fail_tagging)
    enabled_tagging = config.tagging.model_copy(update={"enabled": True, "api_key": "test-key"})
    active_config = config.model_copy(update={"tagging": enabled_tagging})
    buffer = StringIO()
    console = Console(file=buffer, force_terminal=False, color_system=None)
    ...
    assert "sync follow-up tagging failed" in buffer.getvalue()
    assert "Gemini unavailable" in buffer.getvalue()
...
async def test_run_auto_followups_only_reminds_about_incomplete_initial_enrichment(
    ...
    assert "tweetnook import enrich" in output
...
async def test_sync_collection_clears_saved_backfill_after_empty_backfill_page(
    ...
    assert reporter.has_step("sync:bookmarks:head")
    assert not reporter.has_step("sync:bookmarks:backfill")
    assert reporter._step_by_key["sync:bookmarks:head"].summary.endswith(
        "Twitter/X returned no tweets"
    )
...
async def test_sync_failure_releases_lock(paths, config: AppConfig, auth_bundle) -> None:
    ...
    with pytest.raises(TweetNookError):
        await sync_collection(
            "bookmarks",
            full=False,
            limit=None,
...
def test_first_run_creates_dirs_and_missing_auth_is_actionable(
    ...
    monkeypatch.delenv("TWEETNOOK_AUTH_TOKEN", raising=False)
    monkeypatch.delenv("TWEETNOOK_CT0", raising=False)
    monkeypatch.delenv("TWEETNOOK_USER_ID", raising=False)
